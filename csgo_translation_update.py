...
output_zip_path = "workflow/out/CS2-TextMod.zip"

# Create a ZIP archive
with zipfile.ZipFile(output_zip_path, "w", zipfile.ZIP_DEFLATED) as zipf:
    # Write the output file with arcname as "csgo_schinese.txt" (root directory level)
    zipf.write(output_file_path, "csgo_schinese.txt")

    # Write "使用说明.txt" with arcname as the same (root directory level)
    zipf.write("workflow/out/使用说明.txt", "使用说明.txt")

print("任务完成。替换后的文件已保存在 'workflow/out/csgo_schinese.txt' 中。")
